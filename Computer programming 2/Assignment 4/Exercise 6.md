...
#### Header file for class SortableVector:
```cpp
#include "header1.h"
#include <algorithm>

#ifndef SORTABLEVECTOR
#define SORTABLEVECTOR

template <class T> class SortableVector : public SimpleVector<T> {
  using SimpleVector<T>::aptr;
  using SimpleVector<T>::arraySize;
...
template <class T>
SortableVector<T>::SortableVector(int num) : SimpleVector<T>(num) {}

template <class T> void SortableVector<T>::arraySort() {
  sort(aptr, aptr + arraySize);
}

#endif // !SORTABLEVECTOR
```
...
int main() {
  int size;
  cout << "Please input the wanted array size: ";
  cin >> size;
  cout << endl;

  SortableVector<int> testVec = SortableVector<int>(size);
  testVec.push_back(1);
  testVec.push_back(5);
